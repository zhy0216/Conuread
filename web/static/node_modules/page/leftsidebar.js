...
var SubChooser = function(item) {
    ...
    this.feedSiteHeadTemplate = swig.compile($("#feedSiteHeadTemplate").html());

    this.originalFeedSiteDom = $("#feedSiteHead").clone();
    ...
    this.eventBind = function() {
        ...
        $(document).on("click", ".pop-sub-btn", function(e) {
            e.stopPropagation();
            e.preventDefault();
            console.log("click .pop-sub-btn")
            var $this = $(this);
            var feedsiteid = $this.closest("[pop-feedsiteid]").attr("pop-feedsiteid");
            $("#waiter").show();
            $.post("/api/pop-feedsite/sub", {
        ...
        $(document).on("click", "#sidebar [feedsiteid]", function() {
            ...
            if (feedsiteid != "all" && feedsiteid != "star" && self.currentActiveItem.attr("feedsiteid")) {
                self._renderFeedSiteHead()
            } else {
                $("#feedSiteHead").html(self.originalFeedSiteDom.html())
            }
        })

        $(document).on("click", "#sidebar [pop-feedsiteid]", function() {
            var $this = $(this);
            var feedsiteid = $this.attr("pop-feedsiteid");
            self.activeItem($this);
            signal.publish("pop-feedsite-fetch")(feedsiteid);
            var content = self.feedSiteHeadTemplate({})
            $("#feedSiteHead").html(content)
        ...
        // sub-show-menu
        $(document).on("contextmenu", ".sub-show-menu", function(e) {
            console.log("feedsiteid: " + $(this).attr("feedsiteid"))
    ...
    this._renderFeedSiteHead = function() {
        var curFeedSite = this.getCurFeedSite();
        var content = this.feedSiteHeadTemplate({
            site: curFeedSite
        })
        $("#feedSiteHead").html(content)
    }
