...
var ContentShower = function(){
    ...
    this.readFeed = function(dom){
        ...
        var feed = self.feedList.getFeedById(feedid);
        feed.read();
    ...
    this.bindEvent = function(){
        ...
        $("#content").scroll(function(){
            // bottom detect
            // http://stackoverflow.com/a/4638434
            // $("#content").prop("offsetHeight") + $("#content").scrollTop() > $("#content").prop("scrollHeight")
            // TODO: recosider scroll logic

            var $this = $(this);
            var top = $this.offset().top;
            var bottom = top + $this.height();
            var activeLine = top + $this.height()*1/4;
            var done = false;
    ...
    this.scrollNext = function(){
        if(! self.currentActiveFeed){
            self.activeFeed($("#content [feedid]").first());
            return ;
        }
        var $next = self.currentActiveFeed.next();
        var top = $next.position().top;
        var scrollTop = self.targetDom.scrollTop();
        self.scrollTop(scrollTop + top, true);
    ...
    this.toggleStar = function(dom){
        var feedid = dom.closest("[feedid]").attr("feedid");
        var feed = self.feedList.getFeedById(feedid);
        console.log("feedid: "+feedid);
        if(dom.hasClass("glyphicon-star")){
           feed.unstar(function(){
